...
/*        String text = "test message for duplicate test. Test test for test.";
        ...
        List<String> textlist = Arrays.asList("test message for duplicate test. Test test for test.");
        //System.out.println(textlist.stream().map(word -> word.split(" ")).filter(word -> Collections.frequency(textlist, word) > 1)
